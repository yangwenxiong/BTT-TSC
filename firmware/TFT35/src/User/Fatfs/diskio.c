...
)
{
  ...
  if (diskStatus[pdrv] & STA_NOINIT) return RES_NOTRDY;
	...
	{
		case VOLUMES_SD_CARD://SD¿¨
			while (SD_ReadDisk(buff,sector,count))//(SD_Read_Data(buff,sector,count))//¶Á³ö´í  SD_ReadBlock(u8 *buf,long long addr,u16 blksize)
			{
				SD_Init();	//ÖØÐÂ³õÊ¼»¯SD¿¨
			}
      return RES_OK;
...
)
{
  ...
  {
    case VOLUMES_SD_CARD://SD¿¨
      while (SD_WriteDisk((u8*)buff,sector,count))//(SD_Write_Data((u8*)buff,sector,count))//Ð´³ö´í   ;
      {
        SD_Init();	//ÖØÐÂ³õÊ¼»¯SD¿¨
      }
      return RES_OK;
